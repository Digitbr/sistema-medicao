...
function bindReportAutomation() {
  reportForm.elements.tipoManutencao.addEventListener("change", () => {
    updateMaintenanceContexts();
    const maintenanceType = reportForm.elements.tipoManutencao.value;
    if (!["Preventiva", "Corretiva"].includes(maintenanceType)) return;

    const firstEmptyActivity = activityCards.find(
      ({ fields }) => !fields.atividade.value.trim()
    );
    if (!firstEmptyActivity) return;
    firstEmptyActivity.card.classList.add("is-open");
    firstEmptyActivity.fields.atividade.focus();
  });
}

function applyAutomaticDefaults(force = false) {
  const competenceField = reportForm.elements.competencia;
  if (force || !competenceField.value.trim()) {
    competenceField.value = currentCompetence();
  }
  activityCards.forEach((activity) => activity.setAutomaticDates(force));
  updateMaintenanceContexts();
}

function updateMaintenanceContexts() {
  const maintenanceType =
    reportForm.elements.tipoManutencao.value || "Selecione na identificação";
  activityCards.forEach((activity) => activity.updateContext(maintenanceType));
}
...
function currentCompetence(date = new Date()) {
...
function createActivityCards() {
  for (let index = 0; index < 8; index += 1) {
    ...
    const maintenanceTypeLabel = fragment.querySelector("[data-maintenance-type]");
    const activityDateLabel = fragment.querySelector("[data-activity-date]");
    ...
    const setStatus = (status) => {
      ...
      fields.motivo.closest(".waiting-reason").hidden = status !== "em-espera";
      updateSummary();
    };

    const updateDateLabel = () => {
      activityDateLabel.textContent = formatDate(
        fields.dataAntes.value || fields.dataDepois.value || localDateISO()
      );
    };

    const setAutomaticDates = (force = false) => {
      const today = localDateISO();
      if (force || !fields.dataAntes.value) fields.dataAntes.value = today;
      if (force || !fields.dataDepois.value) fields.dataDepois.value = today;
      updateDateLabel();
    };

    const updateContext = (maintenanceType) => {
      maintenanceTypeLabel.textContent = maintenanceType;
      updateDateLabel();
    };
    ...
    const setData = (activity = {}) => {
      ...
      fields.atividade.value = activity.atividade || activity.problema || "";
      fields.conclusao.value =
        activity.conclusao ||
        (activity.status !== "em-espera" ? activity.motivo || "" : "");
      fields.motivo.value =
        activity.status === "em-espera"
          ? activity.motivo || ""
          : activity.motivoEspera || "";
      setAutomaticDates();
    ...
    const getData = async () => {
      ...
      return {
        ...
        conclusao: fields.conclusao.value.trim(),
    ...
    fields.dataAntes.addEventListener("change", updateDateLabel);
    fields.dataDepois.addEventListener("change", updateDateLabel);
...
function resetForm() {
  reportForm.reset();
  reportForm.elements.competencia.value = currentCompetence();
  ...
  return `
    <article class="saved-activity">
      <div class="saved-activity__heading">
        ...
        <div>
          <strong>${escapeHtml(activityDescription(activity))}</strong>
      ...
      ` : ""}
      ${conclusion ? `<p class="saved-conclusion"><strong>Conclusão:</strong> ${escapeHtml(conclusion)}</p>` : ""}
      ${waiting && activity.motivo ? `<p class="saved-waiting-reason"><strong>Motivo da espera:</strong> ${escapeHtml(activity.motivo)}</p>` : ""}
    </article>
  `;
}
                  ...
                  .map(
                    ({ activity, record }) => `
                      <button type="button" class="operations-row" data-open-record="${escapeAttr(record.id)}">
                        <span>
                          <strong>${escapeHtml(recordLabel(record))}</strong>
                          <small>${escapeHtml(activityDescription(activity))}</small>
                        </span>
...
function validateActivities() {
  ...
  const completedWithoutConclusion = filled.find(
    ({ fields }) =>
      fields.status.value === "concluida" && !fields.conclusao.value.trim()
  );
  if (completedWithoutConclusion) {
    completedWithoutConclusion.card.classList.add("is-open");
    completedWithoutConclusion.fields.conclusao.focus();
    throw new Error("Informe a conclusão da atividade finalizada.");
  }
...
function activityDescription(activity) {
  return String(activity?.atividade || activity?.problema || "").trim();
}
