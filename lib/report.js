  ...
  filled.forEach((activity, index) => {
    children.push(
      ...
      new Paragraph({
        spacing: { after: 140 },
        children: [
          new TextRun({ text: "Motivo/observação: ", bold: true }),
...
async function buildPresentationReport(metadata, activities) {
  ...
  filled.forEach((activity, index) => {
    ...
    slide.addText(
      `${activity.status === "em-espera" ? "EM ESPERA" : "CONCLUÍDA"}  |  ${activity.motivo || "Sem observações"}`,
      {
        x: 0.9,
        y: 5.91,
        w: 11.55,
...
function wordPhotoTable(activity) {
  return new Table({
    ...
    rows: [
      new TableRow({
        children: [
          wordPhotoCell("ANTES", activity.fotoAntes, activity.dataAntes),
          wordPhotoCell("DEPOIS", activity.fotoDepois, activity.dataDepois)
        ]
      })
    ]
  });
